...
export function DateRangePicker({ selectedRange, onRangeChange, onClear, label = "Rango de Fechas" }: DateRangePickerProps) {
  ...
  // En pantallas angostas se muestra un solo mes y el calendario ocupa el ancho de la pantalla
  const [esAngosta, setEsAngosta] = useState(() => typeof window !== "undefined" && window.matchMedia("(max-width: 767px)").matches);
  useEffect(() => {
    const mq = window.matchMedia("(max-width: 767px)");
    const cambio = () => setEsAngosta(mq.matches);
    ...
    <div className="relative">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="flex-1 px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-brand-orange focus:border-transparent text-left bg-white dark:bg-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 transition-colors"
        >
      ...
      {isOpen && (
        <>
          ...
          />
          <div className={`${esAngosta ? "fixed left-3 right-3 top-20 max-h-[calc(100dvh-6rem)]" : "absolute top-full left-0 mt-2 max-w-[calc(100vw-2rem)]"} bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-xl z-50 p-4 overflow-auto`}>
            <style>{`
              .rdp {
            ...
            <div className="flex gap-2 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
              <button
                type="button"
                onClick={() => setIsOpen(false)}
                className="flex-1 bg-brand-orange text-white dark:text-gray-900 py-2 rounded-lg hover:bg-brand-navy dark:hover:bg-[#f09b54] transition-colors font-semibold text-sm"
              >
                Aplicar
              </button>
            </div>
